...
// DrawService handles draw-related business logic
...
func (s *DrawService) GetDrawByID(ctx context.Context, id primitive.ObjectID) (*models.Draw, error) {
	return s.drawRepo.FindByID(ctx, id)
}

// GetDrawByDate retrieves a draw by date
func (s *DrawService) GetDrawByDate(ctx context.Context, date time.Time) (*models.Draw, error) {
	return s.drawRepo.FindByDate(ctx, date)
}

// GetDrawsByDateRange retrieves draws by date range with pagination
func (s *DrawService) GetDrawsByDateRange(ctx context.Context, start, end time.Time, page, limit int) ([]*models.Draw, error) {
	return s.drawRepo.FindByDateRange(ctx, start, end, page, limit)
}

// GetDrawsByStatus retrieves draws by status with pagination
func (s *DrawService) GetDrawsByStatus(ctx context.Context, status string, page, limit int) ([]*models.Draw, error) {
	return s.drawRepo.FindByStatus(ctx, status, page, limit)
}

// CreateDraw creates a new draw
func (s *DrawService) CreateDraw(ctx context.Context, draw *models.Draw) error {
...
func (s *DrawService) UpdateDraw(ctx context.Context, draw *models.Draw) error {
	draw.UpdatedAt = time.Now()
	return s.drawRepo.Update(ctx, draw)
}

// DeleteDraw deletes a draw
func (s *DrawService) DeleteDraw(ctx context.Context, id primitive.ObjectID) error {
	return s.drawRepo.Delete(ctx, id)
}

// ScheduleDraw schedules a new draw
func (s *DrawService) ScheduleDraw(ctx context.Context, drawDate time.Time, drawType string, eligibleDigits []int) (*models.Draw, error) {
...
func (s *DrawService) ExecuteDraw(ctx context.Context, drawID primitive.ObjectID) error {
	// Get the draw
...
func (s *DrawService) selectWinners(ctx context.Context, draw *models.Draw, eligibleUsers []*models.User) ([]*models.Winner, error) {
...
func shuffleUsers(users []*models.User) {
	for i := range users {
		j := primitive.NewObjectID().Timestamp().Unix() % int64(i+1)
		users[i], users[int(j)] = users[int(j)], users[i]
	}
}

// GetDrawCount gets the total number of draws
func (s *DrawService) GetDrawCount(ctx context.Context) (int64, error) {
	return s.drawRepo.Count(ctx)
}

// GetDefaultEligibleDigits returns the default eligible digits for a given day of the week
func (s *DrawService) GetDefaultEligibleDigits(dayOfWeek time.Weekday) []int {
	switch dayOfWeek {
	case time.Monday:
		return []int{0, 1}
	case time.Tuesday:
