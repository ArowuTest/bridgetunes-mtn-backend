...
type DrawRepository interface {
	...
	FindNextScheduledDraw(ctx context.Context, afterDate time.Time) (*models.Draw, error) // Added based on build error
}
...
type BlacklistRepository interface {
	IsBlacklisted(ctx context.Context, msisdn string) (bool, error) // Added based on build error
...
type SystemConfigRepository interface {
	...
	Delete(ctx context.Context, key string) error // Note: Implementation uses ID, interface uses key. Decide which is correct.
...
// PointTransactionRepository defines the interface for point transaction operations (New)
type PointTransactionRepository interface {
	...
	// Add other query methods as needed (e.g., by date range)
}

// JackpotRolloverRepository defines the interface for jackpot rollover operations (New)
